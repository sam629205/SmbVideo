...
public class MyMediaController extends FrameLayout implements View.OnClickListener{
	...
	private static final int FORWARD_SEEK = 3;// handler标示   快进指定长度 默认5s
	private static  final int FORWARD_SEEK_LONG_PRESS=4;//// handler标示   快进指定长度 默认5s
	...
	public static String PREFERENCE_FORWARD_INTERVAL = "preferenceForwardInterval";
	public static String PREFERENCE_BUFFER_SIZE = "preferenceBufferSize";
...
//					System.out.println("zh:::+++ SHOW_PROGRESS:SHOW_PROGRESS:SHOW_PROGRESS:");
				...
				case FORWARD_SEEK:
					...
					mPlayerVideoName.setText(StringUtils.generateTime(forwardPosition) + "/" + StringUtils.generateTime(mPlayer.getDuration())+"("+(int)percent+"%) "+speed+"kb/s");
					mTopTools.setVisibility(VISIBLE);
					sendEmptyMessageDelayed(FADE_OUT,1000);
					break;
				case FORWARD_SEEK_LONG_PRESS:
					isIntervalSeeking = true;
					break;
			default:
				break;
			}
	...
	public void initControllerTools(Activity activity, View contentView, StandardGSYVideoPlayer player, PlayerActivity playerActivity) {
		...
		this.seekTimer.schedule(new TimerTask() {
			@Override
			public void run() {
				if (isIntervalSeeking){
					mHandler.sendEmptyMessage(FORWARD_SEEK);
				}
			}
		},0,seekInterval);
	...
	private void initControllerViews(View v) {
		...
		forwardSeekIv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Log.d("forwordseek","click");
				mHandler.sendEmptyMessage(FORWARD_SEEK);
			}
		});
		forwardSeekIv.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				Log.d("forwordseek","longpress");
				mHandler.sendEmptyMessage(FORWARD_SEEK_LONG_PRESS);
				return false;
			}

		});
		forwardSeekIv.setOnTouchListener(new OnTouchListener() {
			@Override
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_UP&&isIntervalSeeking){
					isIntervalSeeking = false;
				}
				return false;
			}
		});
		...
		mLockScreenSwitch.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(isLockedTools){
					...
					forwardSeekIv.setVisibility(VISIBLE);
					isLockedTools = true;
...
//		}
	...
	public void showControllerTools() {
		forwardSeekIv.setVisibility(INVISIBLE);
		int mCurrentOrientation = getResources().getConfiguration().orientation;
		mHandler.removeMessages(FADE_OUT);
		mHandler.removeMessages(FADE_OUT);
